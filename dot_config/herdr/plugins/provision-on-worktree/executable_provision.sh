#!/usr/bin/env sh
# provision.sh — runs `make up` inside a newly created worktree
#
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Running make up in $WORKTREE_DIR"

if make up; then
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] make up succeeded in $WORKTREE_DIR"
else
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] make up failed in $WORKTREE_DIR"
  exit 1
fi
